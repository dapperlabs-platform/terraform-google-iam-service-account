locals {
  ...
  add_github_workload_identity_federation = length(compact([local.github_repository, local.github_environment])) == 2
  github_pool_id                          = local.add_github_workload_identity_federation ? substr("${split("/", local.github_repository)[1]}-${var.name}", 0, 32) : ""
